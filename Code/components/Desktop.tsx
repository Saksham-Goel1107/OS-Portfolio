...
import { TerminalIcon, FolderIcon, UserIcon, FileTextIcon, BriefcaseIcon, CalendarIcon, SettingsIcon, GoelOSIcon } from '@/components/Icons';
    ...
    if (phase === 'off') {
        return (
            <div className="fixed inset-0 flex flex-col items-center justify-center gap-6 z-[9999]" style={{ background: '#000' }}>
                ...
                <div style={{ color: '#444', fontFamily: "'Ubuntu Mono', monospace", fontSize: 13 }}>GoelOS is powered off</div>
                <button
                    onClick={onPowerOn}
...
export default function Desktop() {
    ...
    const openDesktopItem = useCallback((id: string) => {
        ...
        if (item.kind === 'link') {
            if (id === 'email-link') {
                window.location.href = 'mailto:sakshamgoel1107@gmail.com';
            } else if (item.href) {
