...
const INIT: Line[] = [
    { t: 'sys', v: '+--------------------------------------------------+' },
    { t: 'sys', v: '|  GoelOS Terminal  bash 5.2.0                   |' },
...
export default function TerminalApp({ onClose }: { onClose?: () => void }) {
    ...
    const runCmd = useCallback((raw: string): Line[] => {
        ...
        switch (cmd) {
            ...
            case 'pwd':
                out.push(txt(cwd === 'Home' ? '/home/saksham' : `/home/saksham/${cwd}`));
                break;
            ...
            case 'cat': {
                ...
                if (file && !file.dir) {
                    if (f === 'readme.txt') {
                        [txt('# GoelOS Portfolio'), txt(`By ${PORTFOLIO.name} | ${PORTFOLIO.email}`)].forEach(l => out.push(l));
                    } else if (f === '.bashrc') {
            ...
            case 'whoami': out.push(txt('saksham')); break;

            case 'uname':
                out.push(txt(args.includes('-a')
                    ? 'Linux GoelOS 6.5.0-kapoor-generic #1 SMP PREEMPT_DYNAMIC Thu Feb 26 09:00:00 UTC 2026 x86_64 GNU/Linux'
                    : 'Linux'));
                break;
            ...
            case 'sudo':
                out.push({ t: 'err', v: '[sudo] password for saksham:' });
                out.push({ t: 'err', v: 'sudo: Permission denied 😏' });
                break;

            case 'neofetch':
                [
                    { t: 'neo' as LT, v: '         .´ - ´.        saksham@GoelOS' },
                    { t: 'neo' as LT, v: "       /  ( ● )  \\       ──────────────────────" },
                    { t: 'neo' as LT, v: '      |   \\___/   |      OS:      GoelOS 24.04 LTS' },
                    ...
                    { t: 'neo' as LT, v: '          ubuntu        User:    Saksham Goel' },
                ].forEach(l => out.push(l));
                break;
    ...
    const handleKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.ctrlKey && e.key === 'c') {
            ...
            addLines([{
                t: 'mixed', v: '', spans: [
                    { text: `saksham@goelOS:${displayPath}$`, color: TEAL },
            ...
            <div className="flex-1 overflow-y-auto cursor-text scrollbar-ubuntu" style={{ background: BG }}>
                <div className="p-3">
                    ...
                    <div className="flex items-baseline" style={{ fontSize: 13, lineHeight: 1.7, fontFamily: MONO }}>
                        <span style={{ color: TEAL, whiteSpace: 'nowrap' }}>saksham@GoelOS:{displayPath}$&nbsp;</span>
